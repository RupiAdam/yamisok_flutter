...
class _profilefullState extends State<profilefull> {
   List<Container> _listAchievment = new List();
  String _datanew="";
  ...
  //           },
  //         ];
  // _loopingListData()async{
    
     

  //   for(var i = 0; i < datalisnya.length; i++){
  //     print("lihat jalan ini coba");
  //     final  datalisnyadt =datalisnya[i];
    ...
    return MaterialApp(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            ...
            final aciTitile = snapshot.data.result.achievements.length;
//            Map achive =  json.decode(snapshot.data.result.achievements);
            List<Achievement> achievements = snapshot.data.result.achievements;
             List<ListMission> listMission = snapshot.data.result.listMission;
            // return _buildListView(profiles);
           
            final achievementsdt =achievements;
            final lihatleght = achievementsdt.length;
            // final title = ceklen[0].title;

            // for(var i = 0; i < achievementsdt.length; i++){
            //   final  datalisnyadt =achievementsdt[i];
            //   print('lihat loop $datalisnyadt $lihatleght');
            // }
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  children: <Widget>[
                    ...
                    _buildAchievementnew(achievementsdt),
...
//                    Achievementfull(),
                    ...
                    SparatorHeaderdaily(),
                    // DailyMission(),
                     _buildDailyMission(listMission),
                    SparatorHeaderMatch(),
                    UpcomingMatch(),
                  ],
                ),
  ...
  Widget _buildAchievementnew(List<Achievement> achievementsdt) {

    final ceklen = achievementsdt; 
    // var size = globals.varMediaQuery;
     var size = MediaQuery.of(context).size;
    ...
    final double itemWidth2 = (size.width / 3) * 2;
        ...
        var listViewdt = new 
            ...
            Container( 
              ...
              shrinkWrap: true,
              itemCount: ceklen.length,
              itemBuilder: (context, index) {
                final title = ceklen[index].title;
                final description = ceklen[index].description;
                final avatarUrlnew = ceklen[index].avatarUrl;
                return 
                Container(
                  padding: const EdgeInsets.only(
                      left: 5.0, right: 5.0, top: 5.0, bottom: 5.0),
  ...
  Widget _buildDailyMission(List<ListMission>listMission){
    final _dtMission = listMission; 
    // var size = globals.varMediaQuery;
     var size = MediaQuery.of(context).size;
    ...
    final double itemWidthDaily = itemWidth / 2.5;
        ...
        var listViewdt = new 
            ListView.builder(
              scrollDirection: Axis.vertical,
              shrinkWrap: true,
              itemCount: _dtMission.length,
              itemBuilder: (context, index) {
                final _title = _dtMission[index].title;
                final _cookies = _dtMission[index].cookies;
                final _exp = _dtMission[index].exp;
...
class Editprofilelama extends StatefulWidget {
  @override
  _EditprofilelamaState createState() => _EditprofilelamaState();
}

class _EditprofilelamaState extends State<Editprofilelama> {
...
class SparatorHeaderAchivement extends StatelessWidget {
 ...
 Widget _buildAchievement(List<Achievement> achievementss) {
     
        final ceklen = achievementss; 
        ...
        var listView = new ListView.builder(
              ...
              itemBuilder: (context, index) {
                final title = ceklen[index].title;
                final desc = ceklen[index].description;
                final avatarUrl = ceklen[index].avatarUrl;
                return Card(
                  child: Container(
                    width: 100.0,
                    height: 70.0,
                    decoration: BoxDecoration(color: Color.fromRGBO(64, 75, 96,1)),
                    child: ListTile( 
