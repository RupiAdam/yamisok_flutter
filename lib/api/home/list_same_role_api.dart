...
class ApiSameRole {
    ...
    print('lihat body dan parse same role  $url ');
    final response = await client.get(
      "$url",
       headers: {"token": userToken, "playerid":playerId }
    );
   final statusCode = response.statusCode;
    print('lihat status code api list game same gameers $statusCode');
    if (statusCode==200) {
