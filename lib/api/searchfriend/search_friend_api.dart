...
class ApiSearchFriend {
    static Future<String> apisearchfriend(offset,limit,idgame,skillid,keyUsername) async {
    final bool statusApi = globals.statusApi;
    final String baseUrl = globals.baseUrlApi;
    final String baseUrlPro = globals.urlApiPro;
    var urllink="v1/search/search-with-filter?pid";
    var url="";
    ...
    final idkota = await keyStore.getidKotaForSearchMabar();
    final idgender= await  keyStore.getGenderForSearch();
    
    
    if(statusApi==false){
      url="$baseUrl/$urllink=$playerId&cid=$idkota&key=$keyUsername&gid=$idgame&gender=$idgender&sid=$skillid&offset=$offset&limit=$limit";
    }else{
      url="$baseUrlPro/$urllink=$playerId&cid=$idkota&key=$keyUsername&gid=$idgame&gender=$idgender&sid=$skillid&offset=0&limit=10";
    }
   ...
   final statusCode = response.statusCode;
    print('lihat status code api search game $statusCode');
    if (statusCode==200) {
